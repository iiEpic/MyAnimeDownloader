...
class WCOStream(object):
    ...
    def check_output(self, anime_name):
        if self.output is None:
            if self.settings.get_setting('useKnownDownloadLocation'):
                if anime_name in self.output_saver.saved_location:
    ...
    def download_show(self):
        ...
        for episode in self.settings.create_link(self.url, self.season, self.ep_range):
            print(episode)
        exit(1)
        ...
        if season != "season-All" and ep_range != "All":
            ...
            if season == 'season-1':
                matching = [s for s in links if ('season' not in s or season in s) and 'episode' in s]
            else:
